...
const LocateCustomer = () => {
  ...
  const [redirectToVendorPay, setRedirectToVendorPay] = useState(false);
  ...
  async function sendData() {
    ...
    let dateN = new Date();
    console.log(parseInt(dateN.getHours() + '' + dateN.getMinutes()));

    let bodyContent = JSON.stringify({
      // rings: [selected.pinCode, parseInt(dateN.getHours()+10 + '' + dateN.getMinutes()), todayDay.id],
      rings: [selected.pinCode, parseInt(10 + '' + "56"), todayDay.id],
    });

    let response = await fetch('https://techthe.geetika1210.repl.co/result', {
      method: 'POST',
      body: bodyContent,
      headers: headersList,
    });
        ...
        <div className="grid grid-cols-5 px-10 gap-5 py-10">
          <div className="col-span-2 flex flex-col gap-6">
            ...
            <div className="border-b-4 border-gray-300 pb-10">
              <button
                className="text-xl font-bold bg-green-500 px-4 py-2 rounded-2xl"
                onClick={sendData}
              >
                Submit Data
              </button>
            ...
            {fetchComplete && (
            <div className="">
              <h2 className="text-2xl font-semibold font-public-sans">
                The <span className="text-green-600 text-3xl">probability</span> to get sales at this point of time at this location is {probability}%
              </h2>
            </div> )}
          </div>
